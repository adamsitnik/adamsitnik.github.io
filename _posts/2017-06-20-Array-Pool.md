---
layout: post
title: Pooling large objects with ArrayPool
...
Is it a problem for your application? You can perform an initial validation by using the built-in `perfmon.exe`.
...
As you can see it's not a problem for my Visual Studio (#Gen 2 Collections is very low compared to Gen 0/1).

## The Solution

The solution is very simple: buffer pooling. Pool is a set of initialized objects that are ready to use. Instead of allocating a new object, we rent it from the pool. Once we are done using it, we return it to the pool. Every large managed object is an array or an array wrapper (string contains a lenght field and an array of chars). So **we need to pool arrays to avoid this problem**. 
...
Let's use BenchmarkDotNet and compare the cost of allocating arrays with the `new` operator vs pooling them with `ArrayPool<T>`. To make sure that allocation benchmark is including cost of GC, I am configuring BenchmarkDotNet explicity not to force GC collections. Pooling is combined cost of `Rent` and `Return`. I am running the benchmarks for .NET Core 2.0, which is important because it's faster version of ArrayPool (for .NET Core 2.0 it is built-in to runtime, whereas older frameworks use corefx version).
...
If you are not familiar with the output produced by BenchmarkDotNet with enabled Memory Diagnoser, you can read my [dedicated blog post](http://adamsitnik.com/the-new-Memory-Diagnoser/#how-to-read-the-results) to find out how to read these results.
...
```
 |               Method | SizeInBytes |           Mean |          Error |          StdDev |    Gen 0 |    Gen 1 |    Gen 2 |  Allocated |
 |--------------------- |------------ |---------------:|---------------:|----------------:|---------:|---------:|---------:|-----------:|
 |             Allocate |         100 |       **8.078 ns** |      0.0131 ns |       0.0110 ns |   0.0610 |        - |        - |      128 B |
 | RentAndReturn_Shared |         100 |      **44.219 ns** |      0.0336 ns |       0.0314 ns |        - |        - |        - |        0 B |
...
For very small chunks of memory the default allocator can be faster.
 ...
 |               Method | SizeInBytes |           Mean |          Error |          StdDev |    Gen 0 |    Gen 1 |    Gen 2 |  Allocated |
 |--------------------- |------------ |---------------:|---------------:|----------------:|---------:|---------:|---------:|-----------:|
 |             Allocate |        1000 |      **41.330 ns** |      0.0932 ns |       0.0826 ns |   0.4880 |   0.0000 |        - |     1024 B |
 | RentAndReturn_Shared |        1000 |      **43.739 ns** |      0.0380 ns |       0.0337 ns |        - |        - |        - |        0 B |  
 
For 1 000 bytes they are almost on par.

 |               Method | SizeInBytes |           Mean |          Error |          StdDev |    Gen 0 |    Gen 1 |    Gen 2 |  Allocated |
 |--------------------- |------------ |---------------:|---------------:|----------------:|---------:|---------:|---------:|-----------:|
 |             Allocate |       10000 |     **374.564 ns** |      5.1894 ns |       4.6002 ns |   4.7847 |   0.0000 |        - |    10024 B |
 | RentAndReturn_Shared |       10000 |      **44.223 ns** |      0.0399 ns |       0.0333 ns |        - |        - |        - |        0 B |

The bigger it gets, the slower it takes to allocate the memory.

 |               Method | SizeInBytes |           Mean |          Error |          StdDev |    Gen 0 |    Gen 1 |    Gen 2 |  Allocated |
 |--------------------- |------------ |---------------:|---------------:|----------------:|---------:|---------:|---------:|-----------:|
 |             Allocate |      100000 |   3,637.110 ns |     15.9844 ns |      13.3477 ns |  31.2497 |  31.2497 |  **31.2497** |   100024 B |
 | RentAndReturn_Shared |      100000 |      46.649 ns |      0.0414 ns |       0.0346 ns |        - |        - |        - |        0 B |
...
Gen 2 collections! `100 000 > 85 000`, so we get our first Full Garbage Collections!
 ...
 | RentAndReturn_Shared |       10000 |      **44.223 ns** |      0.0399 ns |       0.0333 ns |        - |        - |        - |        0 B |
 | RentAndReturn_Shared |      100000 |      **46.649 ns** |      0.0414 ns |       0.0346 ns |        - |        - |        - |        0 B |
 | RentAndReturn_Shared |     1000000 |      **42.423 ns** |      0.0703 ns |       0.0623 ns |        - |        - |        - |        0 B |
...
But what happens if you try to rent a buffer, that exceeds the max array length of given pool (2^20)?
 ...
 |               Method | SizeInBytes |           Mean |          Error |          StdDev |    Gen 0 |    Gen 1 |    Gen 2 |  Allocated |
 |--------------------- |------------ |---------------:|---------------:|----------------:|---------:|---------:|---------:|-----------:|
 |             Allocate |    10000000 | 557,963.968 ns | 38,932.3180 ns | 114,792.8181 ns | 211.5625 | 211.5625 | 211.5625 | 10000024 B |
 | RentAndReturn_Shared |    10000000 | **651,147.998 ns** | 34,036.1013 ns |  98,744.9016 ns | 207.1484 | 207.1484 | 207.1484 | 10000024 B |
 |  RentAndReturn_Aware |    10000000 |      47.033 ns |      0.0641 ns |       0.0600 ns |        - |        - |        - |        0 B |
